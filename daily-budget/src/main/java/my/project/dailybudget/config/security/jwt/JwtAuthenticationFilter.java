package my.project.dailybudget.config.security.jwt;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        String roleName = jwtService.extractRoleName(jwt);
        ...
        if (subject != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            if (roleName.startsWith("USER")) {
                ...
                if (jwtService.isTokenValid(jwt, subject)) {
                    UsernamePasswordAuthenticationToken authenticationToken =
                            new UsernamePasswordAuthenticationToken(
                                    subject, null, List.of(new SimpleGrantedAuthority("ROLE_" + roleName))
                            );
                    authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                    SecurityContextHolder.getContext().setAuthentication(authenticationToken);
                }
    ...
    private boolean isAdminRole(String roleName) {
        for (RoleName adminRole : RoleName.values()) {
            if (adminRole.name().equals(roleName)) {
                return true;
            }
        }
        return false;
